		...
		describe("patch new plant tests", () => {
			test("returns 201 and posts a new plant to the userPlants array", () => {
				return request(app)
					...
					.expect(201)
					.send({ name: "pumpkin", category: "vegetables" })
					.then(({ body }) => {
						...
						expect(body.currentWeek).toEqual([
			...
			test("returns 201 but does not update the userPlants array when passed a plant the person has logged before", () => {
				return request(app)
					.patch("/api/users/georgia123/plants")
					.expect(201)
					.send({ name: "squash", category: "vegetables" })
					.then(({ body }) => {
						expect(body.userPlants).toEqual([
							{ name: "brussels sprouts", category: "vegetables" },
							{ name: "pumpkin seeds", category: "seeds" },
						...
						body.users.forEach((user) => {
							expect(user).toEqual(
								expect.objectContaining({
									...
									name: expect.any(String),
									currentWeek: expect.any(Array),
									badges: expect.any(Array),
									streak: expect.any(Object),
									userPlants: expect.any(Array),
								})
							);
						});
					});
